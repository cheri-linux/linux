...
void __init cheri_init(void)
{
	...
	ddc = (uintcap_t)cheri_getdefault();

	swap_restore_cap = ddc;
	kernel_data_cap = ddc;
	userspace_cap = ddc;
	...
	cheri_print_cap("krnl: ", kernel_data_cap);
	cheri_print_cap("user: ", userspace_cap);
}
